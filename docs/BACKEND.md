...
| GET | `/api/battery/soh?ratedAh=&days=` | State of Health estimado del banco de batería. El Daly BMS/bridge no reporta capacidad de fábrica ni ciclos (y el firmware no se toca), así que se calcula por conteo de culombios sobre `bms_readings`: busca tramos de descarga continua donde el SoC cae ≥15 puntos, integra la corriente (Ah) en ese tramo y lo normaliza a "Ah que representaría una descarga del 100%". Devuelve la mediana de esos tramos (`estimated_full_capacity_ah`) comparada contra `ratedAh` (la "Capacidad (Ah)" configurada en Ajustes de `battery.html`, mismo valor que usa el cálculo de "Ah Gastados"). `days` (default 90, máx 365) controla la ventana de histórico consultada — la subconsulta toma las 300k filas más recientes dentro de esa ventana antes de ordenar ascendente, para no descartar por accidente las lecturas más nuevas si el rango pedido tiene más filas que el límite. Si no hubo ningún tramo de descarga suficientemente amplio, responde `soh_percent: null` — no es un dato nativo del BMS, es una estimación por software (`computeSohFromRows` en `index.js`, con tests en `test/soh.test.js`) |
